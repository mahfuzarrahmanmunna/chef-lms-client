...
// FIX: Validation to ensure JWT_SECRET exists
if (!JWT_SECRET) {
  throw new Error(
...
export function createToken(user: User): string {
  return jwt.sign(
    ...
    JWT_SECRET as string, // FIX: Explicitly cast to string
    { expiresIn: "7d" },
  );
}
...
export function verifyToken(token: string): TokenPayload | null {
  try {
    // FIX: Cast to 'unknown' first, then to 'TokenPayload' to satisfy strict type checking
